import React from 'react'
import Header3 from '../../Componnents/Header3'
import './RescuePage.css';
export default function RescuePage() {
  return (
    <div className='resPage'>
      <Header3 />
      <div className=' resscroll'>
      <div className= "containerdrop">
  <div className="selectdrop">
    <select className='seldrop'>
      <option value="1">Near By Your Current Location</option>
      <option value="2">Near By Coimatore</option>
    </select>
  </div>
        </div>
        <div className='row row-cols-1 row-cols-md-2  row-cols-lg-3 g-4 m-4 '>
          <div className='col'>
            <div className="card">
              <center>
                <span className='textStat'>Rescued</span>
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS-CUsI3VDX0CC0A-ugu6lrLRTVh1uXc4ymn-pIV2bHMg&s" className="card-img-top " alt="..."></img>
                <div className="card-body">
                  <h5 className="card-title resh5">MISSED DOG</h5>
                  <p className='card-text resp mb-2'> Near RR Hospital </p>
                  <p className='card-text resp mb-2'> Sundharapuram Coimatore</p>
                  <p className='card-text resp '> 8778432734</p>
                  <div className='d-flex justify-content-around mb-5'>
                    <button className='btn-default btnrs1'>CANCEL</button>
                    <button className='btn-default btnrs1'>RESCUE</button>
                  </div>
                </div>
              </center>
            </div>
          </div>

          <div className='col'>
            <div className="card">
              <center>
                <span className='textStatRed'>Pending</span>
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSiYqUWHFe2gRaEEMhV_gm3L_43aUAFpkFADA&usqp=CAU&reload=on" className="card-img-top " alt="..."></img>
                <div className="card-body">
                  <h5 className="card-title resh5">NEGLECTED DOG</h5>
                  <p className='card-text resp mb-2'> Near RR Hospital </p>
                  <p className='card-text resp mb-2'> Sundharapuram Coimatore</p>
                  <p className='card-text resp '> 8778432734</p>
                  <div className='d-flex justify-content-around mb-5'>
                    <button className='btn-default btnrs1'>CANCEL</button>
                    <button className='btn-default btnrs1'>RESCUE</button>
                  </div>
                </div>
              </center>
            </div>
          </div>

          <div className='col'>
            <div className="card">
              <center>
              <span className='textStatOr'>Processing</span>
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR8amHMP4E7fiYDtXGomC6yuYeC5B0TrxGvYw&usqp=CAU" className="card-img-top " alt="..."></img>
                <div className="card-body">
                  <h5 className="card-title resh5">ABUSED DOG</h5>
                  <p className='card-text resp mb-2'> Near RR Hospital </p>
                  <p className='card-text resp mb-2'> Sundharapuram Coimatore</p>
                  <p className='card-text resp '> 8778432734</p>
                  <div className='d-flex justify-content-around mb-5'>
                    <button className='btn-default btnrs1'>CANCEL</button>
                    <button className='btn-default btnrs1'>RESCUE</button>
                  </div>
                </div>
              </center>
            </div>
          </div>
          <div className='col'>
            <div className="card">
              <center>
              <span className='textStatRed'>Pending</span>
                <img src="https://media.4-paws.org/7/a/c/e/7acec6f0a01266dc4871a4450d50cb282b9c8273/VIER%20PFOTEN_2020-01-18_005-2641x1827-1920x1328.jpg" className="card-img-top " alt="..."></img>
                <div className="card-body">
                  <h5 className="card-title resh5">WOUNDED DOG</h5>
                  <p className='card-text resp mb-2'> Near RR Hospital </p>
                  <p className='card-text resp mb-2'> Sundharapuram Coimatore</p>
                  <p className='card-text resp '> 8778432734</p>
                  <div className='d-flex justify-content-around mb-5'>
                    <button className='btn-default btnrs1'>CANCEL</button>
                    <button className='btn-default btnrs1'>RESCUE</button>
                  </div>
                </div>
              </center>
            </div>
          </div>
          <div className='col'>
            <div className="card">
              <center>
              <span className='textStat'>Rescued</span>
                <img src="data:image/jpeg;base64," className="card-img-top " alt="..."></img>
                <div className="card-body">
                  <h5 className="card-title resh5">MISSED DOG</h5>
                  <p className='card-text resp mb-2'> Near RR Hospital </p>
                  <p className='card-text resp mb-2'> Sundharapuram Coimatore</p>
                  <p className='card-text resp '> 8778432734</p>
                  <div className='d-flex justify-content-around mb-5'>
                    <button className='btn-default btnrs1'>CANCEL</button>
                    <button className='btn-default btnrs1'>RESCUE</button>
                  </div>
                </div>
              </center>
            </div>
          </div>
          <div className='col'>
            <div className="card">
              <center>
              <span className='textStatOr'>Processing</span>
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTt89nu5HcX65UCAUlQG6ZRP6UDiqelNSnTJg&usqp=CAU" className="card-img-top " alt="..."></img>
                <div className="card-body">
                  <h5 className="card-title resh5">MISSED DOG</h5>
                  <p className='card-text resp mb-2'> Near RR Hospital </p>
                  <p className='card-text resp mb-2'> Sundharapuram Coimatore</p>
                  <p className='card-text resp '> 8778432734</p>
                  <div className='d-flex justify-content-around mb-5'>
                    <button className='btn-default btnrs1'>CANCEL</button>
                    <button className='btn-default btnrs1'>RESCUE</button>
                  </div>
                </div>
              </center>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
